...
function sameTenant(actor, subject, context = {}) {
...
function sameSchool(actor, student) {
...
function sharesClass(actor, student) {
...
export function isVerifiedGuardianRelationship(relationship, { actorId, studentId, schoolId, tenantId } = {}) {
  if (!relationship) return false;
  if (!VERIFIED_GUARDIAN_STATES.has(relationship.state)) return false;
...
export function canAccessStudentData(actor, student, context = {}) {
  ...
  if (GUARDIAN_ROLES.has(role)) {
    const relationship = context.relationship || asArray(context.relationships).find((candidate) =>
...
export function requireVerifiedGuardianRelationship(actorId, studentId, context = {}) {
...
export function requireSchoolAuthorizedRelationship(actorId, studentId, schoolId, context = {}) {
...
export function requireStudentScope(actor, student, context = {}) {
...
export function requireAdminScope(actor, action, context = {}) {
