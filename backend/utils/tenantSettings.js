...
export function normalizeRetention(settings = {}) {
  ...
  const dsarExportDays = Number(
    retention.dsar_export_days ?? retention.dsarExportDays ?? DEFAULT_RETENTION.dsarExportDays
  );
  ...
  const notificationDays = Number(
    retention.notification_days ?? retention.notificationDays ?? DEFAULT_RETENTION.notificationDays
  );
  const aiInteractionDays = Number(
    retention.ai_interaction_days ?? retention.aiInteractionDays ?? DEFAULT_RETENTION.aiInteractionDays
  );

  return {
    auditLogDays: Number.isFinite(auditLogDays) && auditLogDays > 0 ? auditLogDays : DEFAULT_RETENTION.auditLogDays,
    dsarExportDays:
      Number.isFinite(dsarExportDays) && dsarExportDays > 0 ? dsarExportDays : DEFAULT_RETENTION.dsarExportDays,
    analyticsDays:
      Number.isFinite(analyticsDays) && analyticsDays > 0 ? analyticsDays : DEFAULT_RETENTION.analyticsDays,
    notificationDays:
      Number.isFinite(notificationDays) && notificationDays > 0
        ? notificationDays
        : DEFAULT_RETENTION.notificationDays,
    aiInteractionDays:
      Number.isFinite(aiInteractionDays) && aiInteractionDays > 0
        ? aiInteractionDays
        : DEFAULT_RETENTION.aiInteractionDays
  };
}
