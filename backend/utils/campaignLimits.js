...
/**
 * Checks if a proposed campaign violates equity or load limits.
 * @param {string} tenantId
 * @param {string} category
 * @returns {Promise<{allowed: boolean, reason?: string}>}
 */
export async function checkCampaignLimits(tenantId, category) {
  if (!tenantId || category === 'emergency') {
    return { allowed: true };
  }

  const limitRule = CAMPAIGN_LIMITS[category];
  ...
  const result = await query(sql, [tenantId, category, limitRule.window_days]);
