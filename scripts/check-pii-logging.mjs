...
const DANGEROUS_ARG_RE =
  /(password|secret|token|accessToken|idToken|jwt|authorization|authHeader|cookie|message\.body|messageBody|messagePreview|preview\b|body\s*:|input\.body|childName|child\.(?:fullName|name)|studentName|student\.(?:fullName|name)|email|phone|address|prompt|rawPrompt|transcript|vendorPayload|aiPrompt)/i;
