...
function parseCsv(text) {
  ...
  try {
    ...
    return lines
      ...
      .filter(Boolean)
      .map((line) => {
        // Handle potential CSV quoting issues simply for MVP
  ...
  try {
    ...
    if (normalizedType === 'students') {
      ...
      rawRecords.forEach((record, idx) => {
        ...
        // Note: __lineNumber may not exist if csv-parse doesn't add it; destructuring handles both cases.
      ...
      rawRecords.forEach((record, idx) => {
        ...
        validObjects.push({
          ...
          school_id: effectiveSchoolId,
          external_id: extId,
          // Default class name includes the external ID for better identification.
          // This is a change from the previous default of 'Untitled Class'.
      ...
      try {
        ...
        console.error(
          ...
          { jobId, rosterType, error: e instanceof Error ? e.message : e }
        );
      }
    ...
    // lines 254-256 above. The table can only be one of: sis_roster_students, sis_roster_teachers,
    ...
    // Note: We store up to 50 errors in metadata for debugging, but limit API response
    // to 5 warnings to maintain backward compatibility and avoid overwhelming clients.
