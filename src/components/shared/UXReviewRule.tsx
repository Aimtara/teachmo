...
type UXReviewRuleItem = {
  name: string;
  description: string;
  checkpoints: string[];
};

export const UX_REVIEW_RULES: Record<string, UXReviewRuleItem> = {
  MICRO_INTERACTIONS: {
    name: 'Micro-interactions & Delight',
    description:
      'Continuously look for small animations, haptic feedback, and subtle visual cues that make interactions more intuitive and enjoyable.',
    checkpoints: [
  ...
  MODAL_OPTIMIZATION: {
    ...
    description:
      'Regularly review if modals are the best pattern. For multi-step processes or heavy data entry, consider full-page forms.',
    checkpoints: [
  ...
  FEEDBACK_CLARITY: {
    ...
    description:
      'All toast messages, alerts, and notifications should be concise, actionable, and appear at the right time.',
    checkpoints: [
...
type UXReviewRuleProps = {
  rule: UXReviewRuleItem;
  violations?: number[];
};

export default function UXReviewRule({ rule, violations = [] }: UXReviewRuleProps) {
  return (
      ...
      <div className="flex items-center justify-between">
        ...
        <div
          className={`px-2 py-1 rounded text-xs font-medium ${
            violations.length === 0 ? 'bg-green-100 text-green-800' : 'bg-red-100 text-red-800'
          }`}
        >
        ...
        <ul className="space-y-1">
          {rule.checkpoints.map((checkpoint, index) => (
            <li key={index} className="flex items-start gap-2 text-sm">
              ...
              />
              <span className={violations.includes(index) ? 'text-red-700' : 'text-gray-600'}>{checkpoint}</span>
            </li>
          ))}
        </ul>
      </div>
