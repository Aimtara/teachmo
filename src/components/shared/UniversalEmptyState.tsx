...
const getEmptyStateConfig = (context: string, userType: string): EmptyStateConfig => {
  const configs: Record<string, EmptyStateConfig> = {
    'no-children': {
      icon: Users,
      title: 'Welcome to Teachmo!',
      description:
        'Get started by adding your first child profile to receive personalized activities and parenting guidance.',
      primaryAction: {
    ...
    'no-conversations': {
      ...
      primaryAction: {
        ...
        href: userType === 'teacher' ? createPageUrl('TeacherClasses') : createPageUrl('UnifiedCommunity'),
        icon: Users,
      },
    },
    ...
    'no-achievements': {
      ...
      description:
        'Complete activities, maintain streaks, and engage with the community to earn your first badges.',
      primaryAction: {
      ...
      <CardContent className={`text-center ${sizeClasses[size]}`}>
        <div className="space-y-4">
          ...
          <div className="space-y-2">
            <h3 className={`font-semibold text-gray-900 ${titleSizes[size]}`}>{config.title}</h3>
