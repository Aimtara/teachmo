...
| 4. API boundary | Partial/Improved | Reduced temporary exceptions from 37 to 30 in the May 4 pass by moving AI transparency, directory admin, partner incentives/submissions, execution board, admin audit log, and AI policy simulation calls behind domain modules. A checker ratchet now fails if temporary exceptions exceed 30. | 30 exceptions remain, mostly admin/AI prompt/discover/hook surfaces. |
...
| 6. Static quality | Controlled/Improved | Typecheck, TS ratchet, full Vitest, root backend Jest, backend package Jest, lint ratchet, build, launch checks, and bundle-size ratchet pass. Lint debt remains capped at 940 problems with parser and `no-undef` held at zero. Bundle ratchet tightened to 598 kB total / 23 kB initial / 214 kB largest chunk. | Full lint cleanup and deeper bundle budget decision require owner approval/remediation. |
